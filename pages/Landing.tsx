...
export const LandingPage: React.FC = () => {
  ...
  // We use state to handle image load errors by switching to a fallback URL
  // ensuring the site looks good even if local files are deleted.
  const [images, setImages] = useState({
    beforeAfter: '/before-after.jpg',
    creator: '/creator.jpg'
  });

  const handleImageError = (key: 'beforeAfter' | 'creator') => {
    setImages(prev => ({
      ...prev,
      // Fallback URLs from Unsplash
      [key]: key === 'beforeAfter' 
        ? 'https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?q=80&w=1000&auto=format&fit=crop' // Nice portrait for demo
        : 'https://images.unsplash.com/photo-1573496359142-b8d87734a5a2?q=80&w=400&auto=format&fit=crop' // Professional creator placeholder
    }));
  };
  ...
  return (
    <div className="bg-slate-50">
      ...
      <div className="relative bg-navy-900 overflow-hidden">
        <div className="absolute inset-0 opacity-20 bg-[url('https://images.unsplash.com/photo-1497366216548-37526070297c?q=80&w=1920&auto=format&fit=crop')] bg-cover bg-center mix-blend-overlay"></div>
      ...
      <div className="py-16 bg-slate-50 border-t border-slate-200">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          <div className="relative rounded-xl overflow-hidden shadow-2xl border-4 border-white max-w-lg mx-auto bg-slate-200 aspect-square group cursor-pointer">
            <img 
              src={images.beforeAfter}
              alt="Transformation Example" 
              className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-105"
              onError={() => handleImageError('beforeAfter')}
            />
            <div className="absolute inset-0 flex items-end justify-center pb-4 bg-gradient-to-t from-black/50 to-transparent">
              <span className="text-white font-bold text-lg">AI Generated Professional Headshot</span>
            </div>
          </div>
